...
function sendOtpReport() {
  ...
  const now = new Date();
  const timestamp = formatHourlyReportTimestamp_(cfg, now);
  ...
  if (!isMonday_(cfg, now)) {
    cards.push(buildReportCardElements_(
      cfg,
      spreadsheet,
...
function isMonday_(cfg, date) {
  return Utilities.formatDate(date, cfg.TIME_ZONE, 'u') === '1';
}
